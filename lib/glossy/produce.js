...
var GlossyProducer = function (options) {
  ...
  if (options && options.appName)
    this.appName = options.appName.replace(/\s+/g, '');
...
GlossyProducer.prototype.produce = function (options, callback) {
  ...
  if (options.time instanceof Date && !options.date)
  ...
  var compiledMessage = msgData
    .filter(function (messageElement) {
...
function calculatePrival(values) {
  ...
  if (typeof values.facility == 'string' && !values.facility.match(/^\d+$/)) {
    ...
    parseInt(values.facility, 10) &&
    parseInt(values.facility, 10) < 24
  ) {
  ...
  if (typeof values.severity == 'string' && !values.severity.match(/^\d+$/)) {
    ...
    parseInt(values.severity, 10) &&
    parseInt(values.severity, 10) < 8
  ) {
  ...
  if (!isNaN(pri.severity) && !isNaN(pri.facility)) {
    ...
    pri.str = values.version
      ? '<' + pri.prival + '>' + values.version
      : '<' + pri.prival + '>';
    return pri.str;
  } else {
    return false;
    ...
    for (var key in sdElement) {
      if (Array.isArray(sdElement[key])) {
        for (var i = 0; i < sdElement[key].length; i++) {
          sdElement[key][i] = String(sdElement[key][i]).replace(
            /(\]|\\|")/g,
            '\\$1'
          );
