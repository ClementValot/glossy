...
assert.deepEqual(
  syslogParser.parseSignedCertificate(fullySignedSD['ssign-cert']),
  {
    version: '01',
    hashAlgorithm: 1,
    hashAlgoString: 'SHA1',
    ...
    keyBlob: new Buffer(
      fullySignedSD['ssign-cert']['FRAG'].split(/\s/)[2],
      (encoding = 'base64')
    ),
    thisSignature: new Buffer(
      fullySignedSD['ssign-cert']['SIGN'],
      (encoding = 'base64')
    )
  }
);
