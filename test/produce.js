...
assert.equal(
  msg,
  '<163>1 2009-02-14T00:31:30.00+01:00 localhost sudo 123 - - Test Message',
...
syslogProducer.produce(
  ...
  function (cbMsg) {
    assert.equal(
      cbMsg,
      '<107>1 2009-02-14T00:31:30.00+01:00 127.0.0.1 sudo 419 - - Test Message',
      'Valid message in callback returned'
    );
  }
);
...
BSDProducer.produce(
  ...
  function (cbMsg) {
    assert.equal(
      cbMsg,
      '<107>Feb 14 00:31:30 127.0.0.1 sudo[419]: Test Message'
    );
  }
);
...
assert.equal(
  debugMsg,
  '<151>Feb 14 00:31:30 localhost kill[91]: Debug Message'
);
...
assert.equal(
  noticeMsg,
  '<151>Feb 14 00:31:30 localhost kill[16]: Notice Message'
);
...
assert.equal(
  warnMsg,
  '<167>Feb 14 00:31:30 localhost kill[91]: Warning Message'
);
...
assert.equal(
  criticalMsg,
  '<130>Feb 14 00:31:30 localhost kill[91]: Critical Message'
);
...
assert.equal(
  emergencyMsg,
  '<56>Feb 14 00:31:30 localhost kill[91]: Emergency Message'
);
...
assert.equal(
  structuredMsg,
  '<163>1 2009-02-14T00:31:30.00+01:00 mymachine.example.com evntslog - ID47 [exampleSDID@32473 iut="3" eventSource="Application" eventID="1011" seqNo="1"] BOMAn application event log entry...'
);
...
assert.equal(
  structuredWithArray,
  '<163>1 2009-02-14T00:31:30.00+01:00 mymachine.example.com evntslog - ID47 [origin ip="127.0.1.1" ip="127.0.0.1"] BOMAn application event log entry...'
);
...
assert.equal(
  ...
  '<56>Feb  2 00:31:30 localhost kill[91]: Emergency Message'
);
